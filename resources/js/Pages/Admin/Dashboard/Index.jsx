        ...
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-4">
                    <PremiumPanel accent="emerald" themeCard={themeCard} isDarkMode={isDarkMode} bodyClassName="p-4 space-y-4">
                        <PremiumPanelHeader
                            ...
                            trailing={(
                                <div className="flex items-center gap-2 shrink-0">
                                    ...
                                    <span className="text-xs text-zinc-500 font-semibold tracking-tight whitespace-nowrap">Live Monitoring</span>
                                </div>
                            )}
                        />
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="md:col-span-2 space-y-2">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        {routers.length > 0 ? (
                                            <select
                                                ...
                                                className={`text-[11px] font-semibold px-2 py-1 rounded-lg border max-w-[180px] truncate ${themeSelect}`}
                                            ...
                                            <span className="text-amber-500 font-semibold">Belum ada router</span>
                                        )}
                                ...
                                <div className="h-44 w-full">
                                    {resourceHistory.length === 0 ? (
                                        <div className={`h-full flex items-center justify-center text-xs font-semibold tracking-tight ${themeTextSub}`}>
                                            {routers.length === 0
                                ...
                                <div className="space-y-3 pt-3 border-t border-zinc-200/50 dark:border-zinc-800/50">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-zinc-500 font-medium">
                                        ...
                                        <select
                                            ...
                                            className={`text-[11px] font-semibold px-2 py-1 rounded-lg border max-w-[220px] truncate disabled:opacity-50 ${themeSelect}`}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className={`rounded-xl border p-3.5 ${isDarkMode ? 'border-zinc-800 bg-zinc-900/10' : 'border-zinc-200/60 bg-zinc-50/30'}`}>
                                            <div className="flex items-center gap-1.5 text-xs font-semibold text-sky-650 dark:text-sky-400">
                                                <ArrowDownLeft className="w-4 h-4" />
                                                RX (terima)
                                            </div>
                                            <p className={`text-base font-bold mt-1.5 ${themeTextTitle}`}>
                                                {selectedInterface ? formatSpeedBps(interfaceTraffic.rx_bps) : '—'}
                                            </p>
                                        </div>
                                        <div className={`rounded-xl border p-3.5 ${isDarkMode ? 'border-zinc-800 bg-zinc-900/10' : 'border-zinc-200/60 bg-zinc-50/30'}`}>
                                            <div className="flex items-center gap-1.5 text-xs font-semibold text-violet-650 dark:text-violet-400">
                            ...
                            <div className={`p-4 rounded-xl border flex flex-col justify-between space-y-3.5 ${isDarkMode ? 'bg-zinc-900/10 border-zinc-800/50' : 'bg-zinc-50/30 border-zinc-150'}`}>
                                <div className="space-y-2 text-xs">
                                    <div className="flex justify-between items-center font-medium gap-2">
                                        <span className={themeTextSub}>Router</span>
                                        <span className={`${themeTextTitle} font-semibold truncate max-w-[110px] text-right`} title={serverResources.router_name}>
                                            {serverResources.router_name || '—'}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center font-medium">
                                        <span className={themeTextSub}>RouterOS</span>
                                        <span className={`${themeTextTitle} font-semibold truncate max-w-[110px] text-right`} title={serverResources.os}>
                                            {serverResources.os}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center font-medium">
                                        <span className={themeTextSub}>Identity</span>
                                        <span className={`${themeTextTitle} font-semibold truncate max-w-[110px] font-mono text-right`} title={serverResources.hostname}>
                                            {serverResources.hostname}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center font-medium">
                                        <span className={themeTextSub}>Storage</span>
                                        <span className={`${themeTextTitle} font-semibold`}>{serverResources.disk ?? 0}%</span>
                                    </div>
                                    <div className="flex justify-between items-center font-medium">
                                        <span className={themeTextSub}>Total ODP</span>
                                        <span className={`${themeTextTitle} font-semibold`}>{odpNodeCount} Node</span>
                                    </div>
                                    <div className="space-y-2 pt-2 border-t border-zinc-200/50 dark:border-zinc-800/50">
                                        <div className="flex justify-between items-center text-[11px] font-medium">
                                            ...
                                            <span className="text-emerald-500 font-semibold">{odpUsedPorts}/{odpTotalPorts} ({odpUtilizationPercent}%)</span>
                                        </div>
                                        <div className="w-full bg-zinc-200/70 dark:bg-zinc-850 h-1.5 rounded-full overflow-hidden">
                                            <div
                                ...
                                <div className="text-[10px] text-zinc-500 dark:text-zinc-400 font-medium leading-relaxed pt-2 border-t border-zinc-200/50 dark:border-zinc-800/50">
                ...
                <div className="space-y-4">
                    <PremiumPanel accent="sky" themeCard={themeCard} isDarkMode={isDarkMode} bodyClassName="p-4 space-y-3.5">
                        <PremiumPanelHeader
                            icon={Boxes}
                            accent="sky"
                            isDarkMode={isDarkMode}
                            ...
                            trailing={(
                                <Link
                                    href="/inventory"
                                    className={`text-xs font-medium px-2.5 py-1 rounded-lg border transition-colors ${isDarkMode ? 'border-zinc-800 text-zinc-300 hover:bg-zinc-800' : 'border-zinc-200 text-zinc-700 hover:bg-zinc-50'}`}
                                >
                                    Kelola
                                </Link>
                            )}
                        ...
                        {inventoryLowStockCount > 0 && (
                            <div className={`rounded-xl border px-3 py-2.5 flex items-start gap-2.5 ${isDarkMode ? 'border-amber-500/20 bg-amber-500/5' : 'border-amber-200/60 bg-amber-50/30'}`}>
                                <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                                <p className={`text-xs font-semibold leading-snug ${isDarkMode ? 'text-amber-300' : 'text-amber-800'}`}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {inventoryWatchEntries.map(([key, summary]) => {
                                ...
                                return (
                                    <div
                                        key={key}
                                        className={`rounded-xl border p-3.5 transition-all duration-200 ${isLow
                                            ? (isDarkMode ? 'border-amber-500/20 bg-amber-500/5' : 'border-amber-200 bg-amber-50/30')
                                            : (isDarkMode ? 'border-zinc-800 bg-zinc-900/10' : 'border-zinc-200 bg-white shadow-xs')}`}
                                    >
                                        <div className="flex items-start justify-between gap-2">
                                            <div>
                                                <p className={`text-xs font-semibold tracking-tight ${isLow ? 'text-amber-500' : themeTextSub}`}>
                                                    {summary.label}
                                                </p>
                                                <p className={`text-2xl font-bold mt-1.5 tracking-tight tabular-nums ${themeTextTitle}`}>
                                                    {Number(summary.total_quantity || 0).toLocaleString('id-ID')}
                                                    <span className={`text-xs font-medium ml-1 ${themeTextSub}`}>pcs</span>
                                                </p>
                                            </div>
                                            <Icon className={`w-4 h-4 shrink-0 ${isLow ? 'text-amber-500' : 'text-zinc-400 dark:text-zinc-500'}`} />
                                        </div>
                                        <p className={`text-xs mt-2 ${themeTextDesc}`}>
                        ...
                        {lowStockItems.length > 0 ? (
                            <div className="space-y-2">
                                <p className={`text-xs font-semibold tracking-tight ${themeTextSub}`}>Perlu restock</p>
                                {lowStockItems.slice(0, 4).map((item) => (
                                    <div key={item.id} className={`p-2.5 border rounded-xl text-xs font-semibold flex items-center justify-between gap-2 ${isDarkMode ? 'border-zinc-800/60 bg-zinc-900/10' : 'border-zinc-150 bg-zinc-50/20'}`}>
                                        <span className={`truncate ${themeTextTitle}`}>{item.name}</span>
                                        <span className="text-amber-500 tabular-nums shrink-0 font-bold">{item.quantity}/{item.min_stock}</span>
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <p className={`text-xs ${themeTextDesc}`}>Semua stok ONT & Adaptor dalam batas aman.</p>
                        )}
                        ...
                        {recentInventoryMovements.length > 0 && (
                            <div className="space-y-2 pt-1 border-t border-zinc-200/50 dark:border-zinc-800/50">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <p className={`text-xs font-semibold tracking-tight ${themeTextSub}`}>Stok terakhir berubah</p>
                                </div>
                                {recentInventoryMovements.slice(0, 3).map((movement) => (
                                    <div key={movement.id} className={`p-2.5 border rounded-xl text-xs ${isDarkMode ? 'border-zinc-800/60 bg-zinc-900/10' : 'border-zinc-150 bg-zinc-50/20'}`}>
                                        <div className="flex items-center justify-between gap-2">
                                            <span className={`font-semibold truncate ${themeTextTitle}`}>{movement.item?.name || '—'}</span>
                                        ...
                                        <p className={`mt-1 text-xs truncate ${themeTextDesc}`}>
                                            {movement.type_label}
                    ...
                    <PremiumPanel accent="rose" themeCard={themeCard} isDarkMode={isDarkMode} bodyClassName="p-4 space-y-3.5">
                        <PremiumPanelHeader
                            icon={CreditCard}
                            accent="rose"
                            isDarkMode={isDarkMode}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className={`rounded-xl border p-3.5 ${isDarkMode ? 'border-zinc-800 bg-zinc-900/10' : 'border-zinc-200 bg-white shadow-xs'}`}>
                                <p className={`text-xs font-semibold tracking-tight ${isDarkMode ? 'text-rose-450' : 'text-rose-600'}`}>Belum Lunas</p>
                                <p className={`text-xl font-bold mt-1.5 leading-none ${themeTextTitle}`}>{unpaidCount}</p>
                                <p className={`text-xs font-medium mt-1 ${themeTextDesc}`}>{formatRupiah(unpaidTotal)}</p>
                            </div>
                            <div className={`rounded-xl border p-3.5 ${overdueCount > 0
                                ? (isDarkMode ? 'border-amber-500/20 bg-amber-500/5' : 'border-amber-200 bg-amber-50/30')
                                : (isDarkMode ? 'border-zinc-800 bg-zinc-900/10' : 'border-zinc-200 bg-white shadow-xs')}`}
                            >
                                <p className={`text-xs font-semibold tracking-tight ${overdueCount > 0 ? 'text-amber-500' : themeTextSub}`}>Jatuh Tempo Lewat</p>
                                <p className={`text-xl font-bold mt-1.5 leading-none ${overdueCount > 0 ? 'text-amber-500' : themeTextTitle}`}>{overdueCount}</p>
                                <p className={`text-xs font-medium mt-1 ${themeTextDesc}`}>Invoice terlambat</p>
                            </div>
                            <div className={`rounded-xl border p-3.5 ${isDarkMode ? 'border-zinc-800 bg-zinc-900/10' : 'border-zinc-200 bg-white shadow-xs'}`}>
                                <p className={`text-xs font-semibold tracking-tight ${themeTextSub}`}>Penundaan Aktif</p>
                                <p className={`text-xl font-bold mt-1.5 leading-none ${themeTextTitle}`}>{pendingDeferrals}</p>
                                <p className={`text-xs font-medium mt-1 ${themeTextDesc}`}>Menunggu generate</p>
                            </div>
                            <div className={`rounded-xl border p-3.5 ${isDarkMode ? 'border-zinc-800 bg-zinc-900/10' : 'border-zinc-200 bg-white shadow-xs'}`}>
                                <div className="flex items-center gap-1.5">
                                    <Wifi className={`w-3.5 h-3.5 ${themeTextDesc}`} />
                                    <p className={`text-xs font-semibold tracking-tight ${themeTextSub}`}>Router Aktif</p>
                                </div>
                                <p className={`text-xl font-bold mt-1.5 leading-none ${themeTextTitle}`}>{routerActive}/{routerTotal}</p>
                                <p className={`text-xs font-medium mt-1 ${themeTextDesc}`}>Mikrotik online</p>
                            </div>
                        </div>

                        {(overdueCount > 0 || customerStats.isolated > 0) && (
                            <div className={`rounded-xl border px-3 py-2.5 ${isDarkMode ? 'border-rose-500/25 bg-rose-500/5' : 'border-rose-200 bg-rose-50/30'}`}>
                                <p className={`text-xs font-semibold leading-snug ${isDarkMode ? 'text-rose-350' : 'text-rose-800'}`}>
                    ...
                    <PremiumPanel accent="amber" themeCard={themeCard} isDarkMode={isDarkMode} bodyClassName="p-4 space-y-3.5">
                        <PremiumPanelHeader
                            icon={MessageSquare}
                            accent="amber"
                            isDarkMode={isDarkMode}
                        ...
                        <div className="space-y-2 max-h-[195px] overflow-y-auto pr-1">
                            {waLogs.length === 0 ? (
                                ...
                                waLogs.map((log, idx) => (
                                    <div key={idx} className={`p-3 border rounded-xl flex flex-col gap-1.5 sm:flex-row sm:items-start sm:justify-between text-xs transition-colors duration-150 ${isDarkMode ? 'bg-zinc-900/10 border-zinc-800/50' : 'bg-zinc-50/20 border-zinc-150'}`}>
                                        <div className="space-y-1 min-w-0 flex-1">
                                            <div className="flex flex-wrap items-center gap-2">
                                                <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase border shrink-0 ${isDarkMode ? 'bg-zinc-900 text-zinc-450 border-zinc-800/80' : 'bg-zinc-100 text-zinc-650 border-zinc-200/80'}`}>
                                                    {log.type}
                                                </span>
                                                <span className={`${themeTextTitle} font-semibold truncate`}>{log.target}</span>
                                            </div>
                                            <p className={`${themeTextDesc} text-xs leading-relaxed line-clamp-2 sm:line-clamp-1`}>{log.text}</p>
                                        </div>
                                        <span className={`text-xs ${themeTextSub} font-mono shrink-0 self-end sm:self-auto`}>{log.time}</span>
                                    </div>
                                ))
                            )}
                        </div>
