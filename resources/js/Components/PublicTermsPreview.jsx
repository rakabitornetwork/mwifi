...
export default function PublicTermsPreview({
    ...
    const preview = termsSections.slice(0, maxSections);
            ...
            <div className={`rounded-2xl border p-4 sm:p-5 space-y-4 ${boxClass}`}>
                {preview.map((section) => (
                    <div key={section.title}>
                        ...
                        <p className={`text-xs sm:text-sm mt-1.5 leading-relaxed ${textClass}`}>
                            {section.body}
                        </p>
                    </div>
                ))}
                <p className={`text-xs pt-2 border-t ${variant === 'vps' ? 'border-white/[0.06]' : 'border-slate-800'}`}>
                    Dengan menggunakan layanan ini, Anda menyetujui Syarat & Ketentuan kami.{' '}
